...
export default function FrameImageModal({ frame, videoFile, onClose }: Props) {
  ...
  return (
    ...
    >
      <div className="flex max-h-[90vh] w-full max-w-5xl flex-col overflow-hidden rounded-2xl border border-aegis-border bg-aegis-surface shadow-2xl lg:flex-row">
        <div className="relative flex min-h-0 min-w-0 flex-1 flex-col bg-black/40 p-3">
          <button
            type="button"
            onClick={onClose}
            className="absolute right-4 top-4 z-10 flex h-9 w-9 items-center justify-center rounded-lg border border-white/10 bg-black/50 text-slate-200 transition hover:bg-white/10"
            aria-label="Close"
          >
            <X className="h-4 w-4" />
          </button>
          ...
          {captureError && !displaySrc && (
            <div className="flex aspect-video w-full items-center justify-center px-6 text-center text-sm text-slate-400">
              Could not load this frame. Use backend analysis with stored frame JPEGs, or ensure a
          ...
          {imgNaturalSize && (
            <p className="mt-2 text-center text-[10px] text-slate-500">
              {imgNaturalSize.w}×{imgNaturalSize.h}px · t={frame.timestamp_seconds.toFixed(1)}s
            </p>
          )}
        </div>

        <div className="flex w-full max-w-md shrink-0 flex-col gap-4 border-t border-aegis-border bg-aegis-surface2/90 p-5 lg:border-l lg:border-t-0">
          <div>
            <h2 id="frame-modal-title" className="text-sm font-semibold text-white">
          ...
          <p className="text-[13px] leading-6 text-slate-200">{frame.description}</p>
          ...
          {detections.length > 0 && (
            <div>
              ...
              <ul className="mt-2 space-y-2 text-[12px] text-slate-300">
                {detections.map((d, i) => (
                  <li key={i} className="flex justify-between gap-2 border-t border-aegis-border/60 pt-2 first:border-t-0 first:pt-0">
                    <span>{d.label}</span>
